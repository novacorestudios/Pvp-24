"""Retain explicitly reviewed official Binance CMS listing sources for M11X.

Catalog/search discovery is not evidence. Only exact caller-selected article codes enter this
module. The retained source remains PRELIMINARY and emits no Security row or universe eligibility.
"""
...
SCHEMA = "PVB24_REVIEWED_LISTING_SOURCE_V1"


def inspect_reviewed_listing_source(code, raw):
    ...
    result = {
        ...
        "body_text": body_text,
        "semantic_listing_facts_emitted": False,
...
def retain_reviewed_listing_source(root, code, raw):
    root = Path(root)
    report = inspect_reviewed_listing_source(code, raw)
    ...
    (root / "reports" / f"{report_sha256}.json").write_bytes(encoded)
