# PVB-24 — Participation-Validated Breakout

Independent research and PAPER-only project. **Implementation in progress; not ready for paper or live trading.** No backtest results exist.

Branch: `build/pvb24-v1`. Existing repository display name is `Pvp-24`; GitHub also resolves `pvp-24`. Default main contains initialization only.

Frozen source SHA256: `098a3ca390bce81d506bdec011fc3a936ecbb793f46c2f117f337998bfc1c5d8`

Freqtrade official **2026.8**, exact commit `9f10e357a93c1dcf10c2a2b367659214d89c073e`, Python 3.12; installation pending.

See docs/HANDOFF_STATE.md, docs/PVB24_DECISION_REGISTER.md and docs/REPRODUCE.md. No automatic LIVE trading.
