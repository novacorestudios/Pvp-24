...
A concrete durable PRELIMINARY L2 backend now models entry and records recoverable fills/refusals. Remaining gates include backend protective/exit/cancel execution, full process integration, source-backed market/funding/rules coverage and execution-model qualification. Native Freqtrade dry-run does not preserve the required IOC/reduce-only semantics and is not used as a substitute.
