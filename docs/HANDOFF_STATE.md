...
- Local tests: 68 passed; Ruff lint/format passed. CI for this commit: check GitHub Actions after publication.
...
- Next: verify Milestone 4 CI; then Milestone 5 risk sizing, portfolio reservations, costs and liquidation constraints.
- Code: immutable Fill/Side types, precision-34 Decimal helpers, canonical IDs, SQLite WAL events/snapshots/write-ahead intents, fail-closed paper guard. Causal Timing/Candle/Mark/rule/security models, as-of revision selection, 30-day gap warmup, deterministic historical Top-20 and stale-universe grace implemented. Streaming Wilder ATR, channel/RVOL, exact long/short transitions, restartable indicator checkpoints, cooldown/status gates and timed simultaneous batch ranking implemented. Risk/execution/backtest remain unimplemented.
