# Handoff — Milestone 11G (pre-Final funding coverage and recoverable source evidence)
...
- Local tests: 463 passed; Ruff lint/format passed. CI for this commit: check GitHub Actions after publication.
...
- Next: publish/verify Milestone 11G CI. Then qualify funding interval/schedule semantics without rounding or stitching calc_time, obtain exact source settlement Mark history for the 4291 missing observations, and acquire full historical security/rule snapshots. The 67-month BTCUSDT source audit is complete; do not repeat its downloads. Source gaps are detailed in data/11g-funding-qualification.json and data/11g-metadata-source-review.json. Intraday delisting entry blocking is wired; full universe/rules/settlement-source integration remains incomplete. Archive acquisition is implemented; historical universe/rules, funding schedule/settlement pricing, public feed/account/collateral/liquidation qualification and historical evaluation remain incomplete. Final Test stays locked and operational_ready=false.
