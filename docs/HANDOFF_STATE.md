...
- Local tests: 53 passed; Ruff lint/format passed. CI for this commit: check GitHub Actions after publication.
...
- Next: verify Milestone 3 CI then exact PVB-24 signal engine (Milestone 4).
- Code: immutable Fill/Side types, precision-34 Decimal helpers, canonical IDs, SQLite WAL events/snapshots/write-ahead intents, fail-closed paper guard. Causal Timing/Candle/Mark/rule/security models, as-of revision selection, 30-day gap warmup, deterministic historical Top-20 and stale-universe grace implemented. Signal/risk/execution/backtest remain unimplemented.
