...
## 11F — Reviewed official announcement metadata

Three explicit pre-Final article codes use the official public CMS detail endpoint. Review manifest data/11f-announcement-review.json pins the article body and independently transcribed factual outputs; coverage/provenance and exact raw response hashes are in data/11f-announcement-coverage.json. Raw responses are retained locally under data/announcements-11f/objects and revalidated before event generation. Acquisition is not an original historical receipt; the unknown original revision remains disclosed. A zero source update clock is not normalized to a false timestamp. The source's integer-millisecond publishDate is retained without truncating the seconds shown only approximately on the support page.

The selected response data hash is a65da1c1b329ed60c8dfc1ef726299874859ee2403cbeb4998c733a8a9be6195. Public Git retains factual extracts/hashes, not copyrighted article bodies. Exact repeat replay requires retained original objects and reports. Reacquisition creates a separate revision/report; changing related recommendations changes the raw response identity, but not the selected article's semantic identity when its reviewed body, version and clocks remain unchanged.


## 11G — Multi-month Funding provenance and retained evidence

The final source-selection data hash is 7cede73083b59738ce753df2efbac6fd6ffed1e3fad03d9613eece00197a94d4. Each row of data/11g-funding-source-coverage.json identifies the exact archive attempt/raw hash, API page URLs/object hashes/retrieval clocks, API report hash and immutable monthly audit. Unavailable archive counts remain null. Source data are PRELIMINARY; original publication/receipt times, continuous schedule and position eligibility remain unverified.

Funding history replay verifies complete bounded pagination, source object hashes, cursor continuity, boundary revision consistency, terminal short-page proof and normalized row hashes. The explicit resume summary is hash-pinned and cannot select newly discovered local revisions. Reuse checks the original archive attempts and source pages without repeating network downloads. The retained recovery bundle and its checksums are documented in SOURCE_EVIDENCE_RECOVERY.md.
